        ...
        if (oneClass.students.length === 0) {
          ...
          oneClass.students.forEach((student) => {
            ...
            addGradeButton.addEventListener("click", () => {
              // Pokazuje modal do dodania oceny
              this.gradeModal.style.display = "block";

              // Tworzymy nasłuchiwacz dla tego konkretnego ucznia
              const addGradeForThisStudent = () => {
                // Dodanie oceny do konkretnego ucznia
                this.addGradeToStudent(
                  student.number,
                  this.gradeInput.value,
                  classnameSelected
                );

                // Renderowanie ocen w kontenerze dla tego ucznia
                this.renderGrades(
                  student.number,
                  gradesContainer,
                  student.grades
                ...
                // Zamknięcie modalu po dodaniu oceny
                this.gradeModal.style.display = "none";

                // Usuwamy nasłuchiwacz, aby nie był aktywny po pierwszym kliknięciu
                this.addGradeBtn.removeEventListener(
                  "click",
                  addGradeForThisStudent
                );
              };

              // Przypisanie nasłuchiwacza tylko dla tego ucznia
              this.addGradeBtn.addEventListener(
                "click",
                addGradeForThisStudent
              );
            });
